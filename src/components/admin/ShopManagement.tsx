...
const ShopManagement: React.FC = () => {
    ...
    const getTodayHours = (hours: string) => {
        ...
        return todayLine || lines[0]; // Fallback to first line
    };
    ...
    return (
        <div className="space-y-8 animate-fade-in">
            ...
            <div className="grid grid-cols-1 gap-4">
                {shops.map(shop => (
                    <div key={shop.id} className="bg-white dark:bg-slate-800 p-6 rounded-3xl shadow-sm border border-gray-100 dark:border-slate-700 hover:shadow-md transition-all">
                        <div className="flex justify-between items-start">
                            <div className="flex-1">
                                ...
                                <div className="flex items-center gap-4 text-sm text-gray-500 dark:text-gray-400 mb-4">
                                    ...
                                    {shop.gmbUrl && (
                                        <a href={shop.gmbUrl} target="_blank" rel="noopener noreferrer" className="text-bel-blue hover:underline font-medium">
                                            View on Maps
                                        </a>
                                    )}
                                </div>
                                ...
                                <div className="bg-gray-50 dark:bg-slate-900/50 rounded-xl p-3 max-w-md border border-gray-100 dark:border-slate-700">
                                    ...
                                    {expandedShopId === shop.id && (
                                        <div className="mt-3 pt-3 border-t border-gray-200 dark:border-slate-700 text-sm text-gray-600 dark:text-gray-400 whitespace-pre-line pl-3 border-l-2 border-bel-blue">
                                            {shop.hours}
                                        </div>
                                    )}
                                </div>
